...
# Import the LLM explainer functionality
...
# Function to handle requests that come directly from app.py's process-image endpoint
def handle_tensorflow_request():
    # This is the same logic as the tensorflow_heatmap function below
    return tensorflow_heatmap()
...
def tensorflow_heatmap():
    ...
    try:
        ...
        # For Transformer architecture, class names file is optional
        if architecture == 'Transformer (Google Base)':
            class_names_path = None if not class_names_file else class_names_path
            if class_names_file:
        ...
        # For Transformer architecture, weights file is optional too
        if architecture == 'Transformer (Google Base)':
            weights_path = None
        else:
            if not weights_file:
    ...
    try:
        if architecture in ['ResNet50', 'VGG16', 'InceptionV3']:
            ...
            script_name = 'transformerScript.py'  # Script for Vision Transformer
            args = [image_path]  # Only needs the image path
        else:
        ...
        # Generate explanation using the LLM explainer
        try:
            # Get the original image path
            original_image_path = image_path
            
            # Generate explanation using our LLM explainer
            explanation = generate_explanation(
                original_image_path=original_image_path,
                heatmap_image_path=visualization_image_path,
                predicted_class=predicted_class_name,
                architecture=architecture
            )
            
            logging.info(f"Generated explanation for class '{predicted_class_name}' using {architecture}")
            
            # Encode the visualization image to base64
